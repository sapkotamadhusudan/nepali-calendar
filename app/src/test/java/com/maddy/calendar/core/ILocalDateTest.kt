...
class ILocalDateTest {
    ...
    fun `test monthsBetween`() {
        ...
        assertThat(Period.monthsBetween(start, end)).isEqualTo(ChronoUnit.MONTHS.between(s, e))
        assertThat(Period.monthsBetween(end, start)).isEqualTo(ChronoUnit.MONTHS.between(e, s))
    }
    ...
    fun `test daysBetween`() {
        ...
        assertThat(ILocalDate.daysDifference(start, end)).isEqualTo(Period.daysBetween(start, end))
        assertThat(ILocalDate.daysDifference(end, start)).isEqualTo(Period.daysBetween(end, start))
    ...
    fun `test firstDayOfYear`() {
        ...
        val subMonth = ILocalDate.monthDaysAD().toList().subList(0, today.monthValue - 1)
        ...
        val subMonthBS =
            ILocalDate.monthDaysBS(todayBs.year).toList().subList(0, todayBs.monthValue - 1)
    ...
    fun `test countWeek`() {
        ...
        println(ILocalDate.weekOfMonth(today, com.maddy.calendar.core.DayOfWeek.SUNDAY))
        assertThat(
            Period.weeksBetween(
                today.atStartOfMonth(),
                today,
                com.maddy.calendar.core.DayOfWeek.SUNDAY
            )
        ).isEqualTo(ILocalDate.weekOfMonth(today, com.maddy.calendar.core.DayOfWeek.SUNDAY))


        val weeksCount = ILocalDate.countWeek(today, com.maddy.calendar.core.DayOfWeek.SUNDAY)
        ...
        val javaWeekCount = ChronoUnit.WEEKS.between(
            javaToday.atDay(1).with(TemporalAdjusters.previousOrSame(DayOfWeek.SUNDAY)),
            javaToday.atEndOfMonth().with(TemporalAdjusters.next(DayOfWeek.SUNDAY))
        )
    ...
    fun testDayOfWeekWithJavaTime() {
        ...
        val weekDays = com.maddy.calendar.core.DayOfWeek.weekDays(firstDayOfWeek)
        ...
        val javaWeekDays = com.maddy.calendar.core.DayOfWeek.weekDays(firstDayOfWeek)
            .joinToString(" ") { "  ${it.name.padEnd(10, ' ')}  " }
